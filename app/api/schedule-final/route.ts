...
import { fetchScheduleFinal } from "@/lib/scraper";
import type { ScheduleFinal } from "@/lib/types";
...
let inflight: Promise<ScheduleFinal> | null = null;

async function getScheduleFinal(): Promise<ScheduleFinal> {
  const now = Date.now();
  if (currentCache && currentCache.expiresAt > now) return currentCache.data;
  if (inflight) return inflight;

  inflight = fetchScheduleFinal()
    .then((data) => {
...
export async function GET() {
  try {
    const data = await getScheduleFinal();
