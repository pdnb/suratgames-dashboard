...
/** Stacked bar (CSS): finished / live / pending championship rounds per day. */
export default function DailyFinalsProgressChart({ daily, todayBE }: Props) {
  const maxTotal = daily.reduce((max, d) => Math.max(max, d.finals), 0) || 1;

  return (
    <section
      aria-label="รอบชิงเหรียญทองรายวัน"
      ...
      <div className="relative">
        <div className="grid grid-flow-col auto-cols-fr items-end gap-2 sm:gap-3">
          {daily.map((d) => {
            ...
            const heightPct = (d.finals / maxTotal) * 100;
            const n = d.finals;
            const finishedPct = n ? (d.finishedFinals / n) * 100 : 0;
            const livePct = n ? (d.finalsLive / n) * 100 : 0;
            const pendingPct = 100 - finishedPct - livePct;
            const progress = n
              ? Math.round((d.finishedFinals / n) * 100)
              : 0;
            ...
            return (
              ...
              >
                <span
                  className="inline-flex min-w-7 shrink-0 items-center justify-center rounded-full border border-amber-400/70 bg-amber-100 px-2 py-0.5 text-[10px] font-bold font-mono tabular-nums text-amber-600 shadow-sm dark:border-amber-400/50 dark:bg-amber-500/25 dark:text-amber-50 dark:shadow-amber-900/20"
                  title={`รอบชิง ${d.finals.toLocaleString("th-TH")} รายการ`}
                >
                  {d.finals.toLocaleString("th-TH")}
                </span>
                ...
                <div
                  ...
                  title={`${formatThaiShort(d.dateBE)} — รอบชิง ${d.finals.toLocaleString("th-TH")} | จบ ${d.finishedFinals} | LIVE ${d.finalsLive} | รอแข่ง ${d.finalsPending}`}
                >
                  <div
